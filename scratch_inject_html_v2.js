...
// 1. Extract fallback transcript from youtubeService.ts
const serviceContent = fs.readFileSync('src/services/youtubeService.ts', 'utf-8');
const startMarker = 'const fallbackText = `';
const endMarker = '`;\n\n  if (isTest) {';
const startIndex = serviceContent.indexOf(startMarker);
const endIndex = serviceContent.indexOf(endMarker);

if (startIndex === -1 || endIndex === -1) {
  console.error("Could not find fallbackText in youtubeService.ts!");
  process.exit(1);
}

const fallbackText = serviceContent.substring(startIndex + startMarker.length, endIndex);

// 2. Read current HTML
const htmlPath = 'src/html.ts';
const htmlContent = fs.readFileSync(htmlPath, 'utf-8');

// We want to construct the new html.ts with:
// - Empty url input
// - Hidden subtitles section
// - Empty transcript area
// - Script defining FALLBACK_TRANSCRIPT and handling testSubtitlesBtn click to populate and run generation.

const safeFallbackTextForJS = fallbackText.replace(/`/g, '\\`').replace(/\${/g, '\\${');

const newHtml = `export const html = \`<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
...
<body class="min-h-screen p-4 md:p-8 text-slate-800">
    ...
    <script>
        const FALLBACK_TRANSCRIPT = \`${safeFallbackTextForJS}\`;
                            ...
                            if (line.startsWith('data: ')) {
                                try {
                                    ...
                                    if (data.event === 'progress') {
                                        ...
                                        transcriptArea.readOnly = false; // Allow manual refinement if needed
                                        
                                        const charCount = document.getElementById('subtitleCharCount');
                                        charCount.innerText = "共 " + fetchedTranscript.length + " 字符";
        ...
        async function triggerGenerationWorkflow(transcript) {
            ...
            try {
                const response = await fetch('/api/generate', {
                    ...
                    body: JSON.stringify({ url, transcript, promptRequirements: "" }) // Baseline: no custom prompt
                });
                        ...
                        if (line.startsWith('data: ')) {
                            try {
                                ...
                                if (data.event === 'session') {
                                    ...
                                    inject5W1HButtons('refinedMarkdownOutput', refinedSessionId);
                                } else if (data.event === 'error') {
                                    throw new Error(data.message);
                                }
                            } catch (e) {
        ...
        document.getElementById('testSubtitlesBtn').addEventListener('click', async () => {
            ...
            // Populate fallback link and text
            document.getElementById('url').value = 'https://www.youtube.com/watch?v=4j1omjaRu0A';
            document.getElementById('transcriptArea').value = FALLBACK_TRANSCRIPT;
            document.getElementById('subtitleCharCount').innerText = "共 " + FALLBACK_TRANSCRIPT.length + " 字符";
            ...
            try {
                // Directly call baseline workflow with pre-populated transcript
                await triggerGenerationWorkflow(FALLBACK_TRANSCRIPT);
            } catch (err) {
        ...
        document.getElementById('generateFromCurrentBtn').addEventListener('click', async () => {
            const transcript = document.getElementById('transcriptArea').value.trim();
            if (!transcript) return alert("无可用的字幕内容，请先执行第一步提取或手动粘贴。");
...
console.log("Successfully rewrote src/html.ts!");
